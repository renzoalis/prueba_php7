...
function validarYsubmitear(monto_caja) {
  ...
  /* Valido montos */
    ...
    if(!forma) {
      ...
      switch(forma) {
        case '1':
         monto_ft = $('#input_monto').val();
         if (monto_ft > monto_caja) {
  ...
  // Valido cliente
    if(!transp) {
      alert('Debe ingresar un transportista existente.');
      return false;
    }
...
function getEstadoCC(id) {
  $.ajax({
    ...
    console.log(data);
    $('#deuda_transp').html('');
    $('#deuda_transp').html(data.texto);
    $('#deuda_transp').removeClass().addClass(data.clase);
  }).fail(function(xhr, textStatus, errorThrown) {
      console.log(xhr.responseText);
  }).always(function(){
      // console.log('The ajax call ended.');
