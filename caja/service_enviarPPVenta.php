<?php
   ...
   // $transferencias_detalle -> whereAdd('transf_tipo = 2 AND detalle_ppv != 0 AND detalle_ppv_fh_notif IS NULL AND DATEDIFF("'.date('Y-m-d').'",detalle_liquidado_fh) > 7'); //Tipo = recibida, pppv distinto de 0 y todavia no notificada a ppv
   $transferencias_detalle -> whereAdd('transf_tipo = 2 AND detalle_ppv != 0 AND detalle_ppv_fh_notif IS NULL'); //Tipo = recibida, pppv distinto de 0 y todavia no notificada a ppv
    ...
    foreach ($respuesta as $key => $value) {
        ...
        $transferencia_detalle_update = DB_DataObject::factory('transferencias_detalle');
        $transferencia_detalle_update -> detalle_id = $transferencias_detalle -> detalle_id;
        $transferencia_detalle_update -> find(true);

        $transferencia_detalle_update -> detalle_ppv_fh_notif = date('Y-m-d h:i:s');
        $transferencia_detalle_update -> update();


        //ACTUALIZO EL PPV FINAL PARA EL LOTE
        $producto_stock = DB_DataObject::factory('producto_stock');
        $producto_stock -> ps_id = $transferencias_detalle -> detalle_lote;
        $producto_stock -> find(true);
        $producto_stock -> ps_precio_prom_venta = $transferencias_detalle -> detalle_ppv;
        $producto_stock -> ps_precio_prom_venta_fh = date('Y-m-d h:i:s');
