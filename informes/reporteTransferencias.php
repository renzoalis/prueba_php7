<?php
	...
	$reporte_transferencias = DB_DataObject::factory('view_reporte_transferencias');

	if($_GET['fecha_desde']){
		$reporte_transferencias -> whereAdd('FECHA BETWEEN "'.$_GET['fecha_desde'].'" AND "'.date('Y-m-d H:i:s').'"');
		$reporte_transferencias -> find();
		...
		$reporte_transferencias -> whereAdd('FECHA BETWEEN "'.$f_desde.'" AND "'.$f_hasta.'"');
		$reporte_transferencias -> find();
		$campoFecha = date('d/m/Y',strtotime($f_desde)).' - '.date('d/m/Y');
	}


	require_once('public/reporteTransferencias.html');
	exit;
?>
