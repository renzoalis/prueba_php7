<?php
	...
	$prov -> whereAdd('prov_id = '.$compra -> compra_prov_id);
	$prov -> find(true);
	...
	while ($do_compra_detalle -> fetch()) { 
		if($do_compra_detalle -> ps_precio_prom_venta == 0){		//Si ya tiene calculado el ppv no lo muestro para devolver
			...
			// $detalle[$do_compra_detalle -> detalle_id]['prod_cant'] = $do_compra_detalle -> detalle_prod_cant - $do_compra_detalle -> detalle_prod_dev;
			$detalle[$do_compra_detalle -> detalle_id]['prod_cant'] = $do_compra_detalle -> ps_cantidad;
