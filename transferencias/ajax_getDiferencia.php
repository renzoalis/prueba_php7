<?php
	...
	$do_transferencias -> whereAdd('transf_id = '.$_POST['transf_id']);
	...
	while ($do_transferencia_detalle -> fetch()) { 
		$diferencia = $do_transferencia_detalle -> detalle_producto_cantidad_origen - $do_transferencia_detalle -> detalle_producto_cantidad_destino;

		if($diferencia > $do_transferencia_detalle -> detalle_mercaderia_diferencia){
			$detalle[$do_transferencia_detalle -> detalle_id]['det_id'] = $do_transferencia_detalle -> detalle_id;
			...
			$detalle[$do_transferencia_detalle -> detalle_id]['prod_nombre'] = $do_transferencia_detalle -> prod_nombre.' ('.$do_transferencia_detalle -> prod_alias.')';
			$detalle[$do_transferencia_detalle -> detalle_id]['prod_calibre'] = $do_transferencia_detalle -> detalle_calibre; 
			$detalle[$do_transferencia_detalle -> detalle_id]['prod_cant'] = $do_transferencia_detalle -> detalle_producto_cantidad_origen - $do_transferencia_detalle -> detalle_producto_cantidad_destino - $do_transferencia_detalle -> detalle_mercaderia_diferencia; 
			...
			$detalle[$do_transferencia_detalle-> detalle_id]['total'] = $do_transferencia_detalle-> detalle_producto_cantidad_destino * $do_transferencia_detalle-> detalle_costo_descarga; 
		}

	}

	require_once('public/modales/diferencia_stock.html');
	exit;
?>