<?php
	...
	$do_importadores -> importador_id = $_POST['id'];
	$do_importadores -> find(true);

	$ops = DB_DataObject::factory('cuenta_corriente_operacion');

	$do_cc = DB_DataObject::factory('importador_cuenta_corriente');
	$do_cc -> ccte_importador_id = $_POST['id'];
	$do_cc -> orderBy('ccte_id DESC');
	$do_cc -> limit(20);
	$do_cc -> joinAdd($ops);
	$do_cc -> find();

	$do_cc_last = DB_DataObject::factory('importador_cuenta_corriente');
	$do_cc_last -> ccte_importador_id = $_POST['id'];
	...
	require_once('public/modales/edit_importador.html');
	exit;
?>