<?php
	...
	if(!$_GET['fecha_desde']){
		...
		$do_ventas = $venta -> getVentasTodas($f_desde,date('Y-m-d H:i:s'));
		$campoFecha = date('d/m/Y',strtotime($f_desde)).' - '.date('d/m/Y');
	} else {
		$do_ventas = $venta -> getVentasTodas($_GET['fecha_desde'],$_GET['fecha_hasta']);
	...
	require_once('public/ventas_todas.html');
	exit;
?>
